...
void task1(void)
{
    ...
    while(1)
    {
        ...
        if(queueReceive(&q, &r, 10) == true)
        {
            ...
            Serialprintln("[Tick: %x] [ID: %d] [Priority: %d] [Consumer] Not able to recieve data from queue...", INFO);
    }
}
...
void task2(void)
{
    while(1)
    {
        Serialprintln("[Tick: %x] [ID: %d] [Priority: %d] [Monitor] Queue Front: %d, Queue Rear: %d", INFO, Systick_get_tick(), tcb_list[2].task_id, tcb_list[2].task_priority, q.front, q.rear);
        taskYield();
    }
}
...
void task3(void)
{
    ...
    {
        v++;
        Serialprintln("[Tick: %x] [ID: %d] [Priority: %d] [Producer] Sending %d]", INFO, Systick_get_tick(), tcb_list[3].task_id, tcb_list[3].task_priority, v);

        if(queueSend(&q, &v, 10) == true)
            Serialprintln("[Tick: %x] [ID: %d] [Priority: %d] [Producer] Sent %d", INFO, Systick_get_tick(), tcb_list[3].task_id, tcb_list[3].task_priority, v);
        else
            Serialprintln("[Tick: %x] [ID: %d] [Priority: %d] [Producer] Not able to send %d...", INFO, Systick_get_tick(), tcb_list[3].task_id, tcb_list[3].task_priority, v);
        tim_delay_ms(2000);
    }
}
...
int main(void)
{
    ...
    taskAdd_Priority(&task2, "Task 2", 1);
    taskAdd_Priority(&task3, "Task 3", 2);
