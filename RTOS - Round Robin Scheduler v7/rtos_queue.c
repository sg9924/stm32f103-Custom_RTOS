...
static bool queueAdd(Queue_t* q, const void* data)
{
    ...
    //Buffer Address = Buffer Start + (rear index) * (size of element in bytes)
    uint8_t* dest = (uint8_t*) (_QUEUE_BUFFER_ADDR(q, q->rear));
    //Copy the input data into the calculated buffer address
    xmemcopy(dest, (void*)data, q->element_size);
    //Update rear index and current length
...
static bool queueRemove(Queue_t* q, void* out)
{
    ...
    //Buffer Address = Buffer Start + (front index) * (size of element in bytes)
    uint8_t* source = (uint8_t*) (_QUEUE_BUFFER_ADDR(q, q->front));
    //Copy the data from the calculated buffer address to the output
    xmemcopy(out, source, q->element_size);
    //Update front index and current length
...
//Queue Create - Static Allocation
...
bool queueSend(Queue_t* q, const void* item, uint16_t wait_tick)
{
    ...
    //wait task_node_t is required - static allocation
    task_node_t t;
    ...
    //control will come here when task has been unblocked either
    //due to timeout
    //or space is available in queue
    DISABLE_IRQ();
    ...
    //space still not available, timeout
    else
    {
        ENABLE_IRQ();
        return false;
...
bool queueReceive(Queue_t* q, const void* item, uint16_t wait_tick)
{
    ...
    //queue is still empty, exit due to timeout
    else
    {
        ENABLE_IRQ();
        return false;
