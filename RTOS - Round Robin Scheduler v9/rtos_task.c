...
static bool taskCheck_TickOverflow(uint32_t current, uint32_t target, uint32_t ref)
{
    //current has overflow wrt ref
    if (current < ref)
        //target has overflow or no overflow wrt ref
        return (target < ref && current >= target) || (target > ref);
    //current has no overflow wrt ref
    else
        //target has no overflow wrt ref
        return (current >= target) && (target >= ref);
}
...
//Task Check - Block List for Timeout
//used in Systick to iterate all tasks and unblock valid ones
//Accounts for overflow in Systick and Block Tick
void taskCheck_BlockedListTimeout(void)
{
    ...
    while(t != NULL)
    {
        ...
        //check block tick
        unblock = t->task_state == TASK_STATE_BLOCKED && taskCheck_TickOverflow(current_tick, t->block_tick, t->task_blocked_at);
...
//Task Unblock
//Unblocks a sepcific task by task id
void taskUnblock(uint8_t task_id)
{
    if(task_id > NO_OF_TASKS) return;

    tcb_t* t = &TCBS[task_id];
...
tcb_t* getTask_ByID(uint8_t task_id)
{
    return &TCBS[task_id];
}
